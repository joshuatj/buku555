...
public class UploadServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        String path = request.getServletContext().getRealPath("/")+"imgs/"+ type + "/";
        if (request.getParameter("getfile") != null && !request.getParameter("getfile").isEmpty()) {
            File file = new File(path + request.getParameter("getfile"));
            if (file.exists()) {
                int bytes = 0;
            ...
            File file = new File(path + request.getParameter("getthumb"));
                if (file.exists()) {
                    ...
                    if (mimetype.endsWith("png") || mimetype.endsWith("jpeg")|| mimetype.endsWith("jpg") || mimetype.endsWith("gif")) {
                        ...
                        if (im != null) {
                            BufferedImage thumb = Scalr.resize(im, 100); 
        ...
        try {
            ...
            for (FileItem item : items) {
                if (!item.isFormField()) {
	                    ...
	                    File path = new File(root + "/imgs/" + type);
	                    if (!path.exists())  
	                    {  
                       ...
                       //File file = new File(request.getServletContext().getRealPath("/")+"imgs/", item.getName());
                        ...
                        jsono.put("url", "UploadServlet?type=" + type +"&getfile=" + fileName);
                        jsono.put("thumbnail_url", "UploadServlet?type="+ type +"&getthumb=" + fileName);
                        jsono.put("delete_url", "UploadServlet?type="+ type +"&delfile=" + fileName);
